      ...
      {({ open }) => (
        <div className="flex flex-wrap items-center justify-between w-full lg:w-auto">
          ...
          <DisclosureButton
            ...
            className="px-2 py-1 ml-auto text-muted rounded-md lg:hidden hover:text-primary focus:text-primary focus:bg-primary focus:outline-none dark:text-muted dark:focus:bg-trueGray-700"
          >
            <svg
            ...
            <>
              {navigation.map((item, index) => (
                <Link
                  key={index}
                  href={item.href}
                  className="w-full px-4 py-2 -ml-4 text-muted rounded-md dark:text-muted hover:text-primary focus:text-primary focus:bg-primary dark:focus:bg-card focus:outline-none"
                >
                  {item.text}
                </Link>
              ))}
